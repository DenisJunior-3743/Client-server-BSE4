...
fun RegisterScreen(
    ...
    val fullName = rememberFieldState { if (!Validators.isRequired(it)) "Full name is required" else null }
    ...
    val password = rememberFieldState {
        when {
            ...
            !Validators.isStrongPassword(it) -> "Min 8 characters, with a number and both upper/lower case"
            else -> null
        }
    }
    ...
    Column(
        ...
        Text("Create your account", style = MaterialTheme.typography.headlineSmall, color = AppColors.Neutral900)
        Text(
